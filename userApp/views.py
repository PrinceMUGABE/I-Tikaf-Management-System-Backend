...
def register_user(request):
    print(f"\n submitted user data: {request.data}\n")
    ...
    if not is_admin_creating:
        ...
        print(f"\n Submitted data is: {request.data}\n")
    ...
    try:
        if is_admin_creating:
            ...
            if role != 'participant':
                print({"error": "Can not create this user"})
        ...
        if email:
            message = (
                f"Hello {first_name},\n\nYour account has been created in I'Tikaf Management System.\n"
...
def list_all_users(request):
    ...
    users = CustomUser.objects.all().values(
        'id', 'first_name', 'middle_name', 'last_name', 'phone_number', 'email', 'role', 'status', 'created_at',
    )
...
def get_user_by_email(request):
    ...
    try:
        ...
        if request.user.role != 'imam' and request.user.email != email:
...
def get_user_by_phone(request):
    ...
    try:
        ...
        if request.user.role != 'imam' and request.user.phone_number != phone_number:
            return Response({"error": "You are not authorized to access this user."}, status=403)
